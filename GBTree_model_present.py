...
def model_Tree_result_present(modellr,train_x,train_y,test_x,test_y):
...
def test_parameter_GB(X_train, y_train, test_x, test_y, max_dep, n_estimator):
  print('1：No Injury\n 2:Injury\n 3:Fatal\n')
  ...
  for dep in range(max_dep):
    ...
    cm_test, cm_train, model = model_Tree_result_present(grd,X_train,y_train,test_x,test_y)
...
def build_model_GB(X_train,y_train,test_x,test_y,max_dep, n_estimator):
      result, all_model = test_parameter_GB(X_train, y_train, test_x, test_y, max_dep, n_estimator)
      csv_name = 'GB_maxdep_{}_n_estimator_{}_accuracy.csv'.format(max_dep, n_estimator)
